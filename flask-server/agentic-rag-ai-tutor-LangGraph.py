...
def get_statistics():
    try:
        ...
        if not os.path.exists(SESSION_HISTORY_DIR):
            return jsonify({"error": f"Directory not found: {SESSION_HISTORY_DIR}"}), 500
        if not os.path.exists(COURSE_MATERIAL_DIR):
            return jsonify({"error": f"Directory not found: {COURSE_MATERIAL_DIR}"}), 500

        # Count total number of sessions
        session_files = [name for name in os.listdir(SESSION_HISTORY_DIR) if name.endswith('.txt')]
        ...
        course_dirs = [name for name in os.listdir(COURSE_MATERIAL_DIR) if os.path.isdir(os.path.join(COURSE_MATERIAL_DIR, name))]
